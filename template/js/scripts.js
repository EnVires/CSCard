...
$('.purse').html('Баланс: <span>'+data.balans+' <small>p</small></span>');
	...
	.css('left', '')
	.animate({ left: '-=' + offset }, 250 * data.win_slot, 'easeOutQuad', function(){
				...
				if(rdata.last_drops != null) {
					$.each(rdata.last_drops, function(_, v){
						$('.live-line > div').prepend(
							...
							.addClass('live-line-item '+ v.rare +'')
							.append(
								$('<span>')
								.addClass('line-pic ')
								.css('background-image', 'url(' + v.image + ')')
							...
							.append(
								...
								.append(
									...
									.append('<div class="live-line-item-tooltip-block-source" style="background-image:url(' + v.case_img + ')"></div></a>')
								)
							)
							.css('display', 'none')
							.fadeIn(600)
